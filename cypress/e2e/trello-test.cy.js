describe('Testes de API do Trello', () => {
  it('Deve criar um novo board', () => {
    ...
    cy.request('POST', `/boards/?name=${boardName}&key=${Cypress.env('API_KEY')}&token=${Cypress.env('TOKEN')}`).then(response => {
  ...
  it('Deve criar um novo card em um board', () => {
    ...
    cy.getMyBoards().then(responseBoards => {
      ...
      cy.request('POST', `/lists?name=${listName}&idBoard=${boardId}&key=${Cypress.env('API_KEY')}&token=${Cypress.env('TOKEN')}`).then(responseList => {
        const listId = responseList.body.id; // Pega o ID da lista criada

        // Cria um novo card na lista
        cy.request('POST', `/cards?idList=${listId}&key=${Cypress.env('API_KEY')}&token=${Cypress.env('TOKEN')}`).then(responseCard => {
  ...
  it('Deve excluir um card existente', () => {
    cy.getMyBoards().then(responseBoards => {
      ...
      cy.request('GET', `/boards/${boardId}/lists?key=${Cypress.env('API_KEY')}&token=${Cypress.env('TOKEN')}`).then(responseLists => {
        const listId = responseLists.body[0].id; // Pega o ID da primeira lista do board

        // Pega os cards da lista
        cy.request('GET', `/lists/${listId}/cards?key=${Cypress.env('API_KEY')}&token=${Cypress.env('TOKEN')}`).then(responseCards => {
          const cardId = responseCards.body[0].id; // Pega o ID do primeiro card da lista

          // Exclui o card
          cy.request('DELETE', `/cards/${cardId}?key=${Cypress.env('API_KEY')}&token=${Cypress.env('TOKEN')}`).then(responseCards => {
  ...
  it('Deve excluir um board existente', () => {
    cy.getMyBoards().then(response => {
      ...
      // Exclui o board
      cy.request('DELETE', `/boards/${boardId}?key=${Cypress.env('API_KEY')}&token=${Cypress.env('TOKEN')}`).then(response => {
        expect(response.status).to.eq(200);
      });
    })
  });
